...
/*   Updated: 2023/10/14 18:04:35 by habu-zua         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
char	*return_remaining_txt(char *read_txt)
{
	...
	rem_txt = (char *)malloc(sizeof(char) * (ft_strlen_g(read_txt) - i + 1));
	if (!rem_txt)
		return (NULL);
	i++;
	if (!read_txt[i])
...
char	*ft_strjoin_g(char *s1, char *s2)
{
	...
	if (!s2)
		return (NULL);
	new_str = malloc(sizeof(char) * (ft_strlen_g(s1) + ft_strlen_g(s2) + 1));
	if (!new_str)
		return (NULL);
	i = -1;
	while (s1[++i] != '\0')
